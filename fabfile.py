...
# sudo apt-get curl git, ... 리눅스 설치 명령어
apt_requirements   = [
    'curl',
    'git',
    'python3-dev',
...
# 최초 배포, 업그레이드 배포
# 업그레이드 배포시 명령
# 코드 수정 -> 로컬테스트 완료 -> git 커밋, push -> fab deploy -> 사용서버에 반영완료
# $> fab deploy
def deploy():
...
def _install_apt_requirements(apt_requirements):
    ...
    # reqs => curl git ... 
    # 서비스 구동에 필요한 os레벨상의 패키지를 설치해라
    # $> sudo apt-get 0y install curl git ...
...
def _update_virtualenv():
    ...
    # 해당 가상환경 안에, flask, scikit-learn 2개가 설치 될 것이다.
